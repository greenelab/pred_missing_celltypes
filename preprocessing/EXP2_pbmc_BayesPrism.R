...
combinations <- expand.grid(noise_type = c("nonoise"),
                            proportion_type = c("random"))

# Define the values for num_missing
num_missing_values <- c(0,1,2,3,4)
...
for (num_missing in num_missing_values) {
  ...
  for (i in 1:nrow(combinations)) {
    ...
    proportion_type <- combinations$proportion_type[i]
      ...
      relative_path <- file.path(script_dir, "..", "data", "EXP2/")
      ...
      signal_file <- paste0(import_path, "MCT_", data_type, "_EXP2_", proportion_type,"prop_", noise_type, "_", num_missing, "missing_signal.csv")
      cell_state_file <- paste0(import_path , "MCT_", data_type, "_EXP2_", proportion_type,"prop_", noise_type,"_", num_missing, "missing_cellstate.csv")
      export_file_prop <- paste0(export_path, "MCT_", data_type, "_EXP2_", num_missing, "missing_", proportion_type, "prop_", noise_type, "_InstaPrism_results.csv")
      export_file_ref <- paste0(export_path, "MCT_", data_type, "_EXP2_", num_missing, "missing_", proportion_type, "prop_", noise_type, "_InstaPrism_usedref.csv")
